...
//                            "[game can continue: %B]", root.state_after_move.get().CanGameContinue()));
    ...
    private GameTreeNode which_best(Best best, GameTreeNode selection, boolean use_recursion){
        ...
        if(use_recursion) {
            ...
            heuristic = evaluate(selection, 1) / depth_reached;
        } else {
            heuristic = evaluate(selection, Tuner.max_search_depth - 1);
        }
        if (heuristic > 0) {
    ...
    private double evaluate(GameTreeNode node, int depth){
        double heuristic = 0;
        if (depth < Tuner.max_search_depth) {
            // Recursively sum the best nodes' heuristics
            if (node != null && node.isReady()) {
                depth_reached = depth;
                heuristic = node.combined.get() - node.maximum_sub.get();
                if(heuristic < 0){
                ...
                heuristic += evaluate(find_best_node(node, false), depth + 1);
            } else if (depth == 1) {
