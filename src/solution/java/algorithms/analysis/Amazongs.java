...
//        });
    ...
    static UberDistanceData calculate_all_distances(GameState board) {
        BoardPiece[] p1 = Game.Get().getPrevTurnPieces(board.getRoundNum());
        BoardPiece[] p2 = Game.Get().getTurnPieces(board.getRoundNum());
            ...
            if (neighbours != null) {
                for (Position n : neighbours) {
                    ...
                    if (n.isValid() && board.readTile(n_index) == 0 && distance < distance_map[n_index]) {
                        next_positions[n.getIndex()] = n.getIndex();
    ...
    private static int count_neighbours(GameState board, int tile) {
        ...
        int count = 0;
        for (Position n : neighbours) {
            if (n.isValid() && board.readTile(n.getIndex()) == 0) {
                count++;
            }
        }
        return count;
