...
public class Mobility {
    ...
    public static double CalculateHeuristic(GameState board){
        ...
        double p = board.getRoundNum() / 92.0;
    ...
    public static double CalculateFreedomHeuristic(GameState board){
        return (double)count_first_degree_moves(board, Game.Get().getPrevTurnPieces(board.getRoundNum())) / max_moves;
    }

    public static double CalculateReductionHeuristic(GameState board){
        return 1 - ((double)count_first_degree_moves(board, Game.Get().getTurnPieces(board.getRoundNum())) / max_moves);
    }

    private static int count_first_degree_positions(GameState board, BoardPiece[] pieces) {
        int[] positions = new int[pieces.length];
        for (int i = 0; i < 4; ++i) {
            int index = pieces[i].getIndex();
            positions[i] = index;
        }
        int moves = 0;
